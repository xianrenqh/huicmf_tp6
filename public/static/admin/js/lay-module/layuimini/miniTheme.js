...
layui.define(["jquery", "layer"], function (exports) {
  ...
  var miniTheme = {
    ...
    render: function (options) {
      ...
      miniTheme.changeTheme(window, false, color, second);
    ...
    changeTheme: function (target, autoHead, color, second) {
      miniTheme.autoHead = false;
      miniTheme.changeTheme = function (target, autoHead, color, second) {
        miniTheme.themeColorSet(color, second);
        if (target.frames.length == 0) return;
        for (var i = 0; i < target.frames.length; i++) {
          try {
            if (target.frames[i].layui == undefined) continue;
            target.frames[i].layui.miniTheme.changeTheme(target.frames[i], autoHead, color, second);
          } catch (error) {
            //console.log(error);
          }
        }
