(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
    typeof define === 'function' && define.amd ? define(factory) :
      (global.iceEditor = factory());
}(this, (function () {
  'use strict';
  var ice = ice || {};
  ice.editor = function (id) {
    this.menu = ['backColor', 'fontSize', 'foreColor', 'bold', 'italic', 'underline', 'strikeThrough', 'line', 'justifyLeft', 'justifyCenter', 'justifyRight', 'indent', 'outdent', 'line', 'insertOrderedList', 'insertUnorderedList', 'line', 'superscript', 'subscript', 'createLink', 'unlink', 'line', 'hr', 'face', 'table', 'files', 'music', 'video', 'insertImage', 'removeFormat', 'paste', 'line', 'code'];
    this.notMenu = [];
    ...
    this.imgDomain = 0;
    this.ajax.uploadTimeout = 15000;
    this.ajax.xhr = function () {
    };
    this.ajax.timeout = function () {
    };
    this.ajax.progress = function () {
    };
    ...
    this.face = [{
      title: '文字',
      type: 'text',
      list: [{title: '开心', content: '(^_^)'}, {title: '受不了', content: '(>_<)'}, {
        title: '鄙视',
        content: '(¬､¬)'
      }, {title: '难过', content: '(*>﹏<*)'}, {title: '可爱', content: '(｡◕‿◕｡)'}, {
        title: '无奈',
        content: '╮(╯_╰)╭'
      }, {title: '惊喜', content: '╰(*°▽°*)╯'}, {title: '听音乐', content: '♪(^∇^*)'}, {
        title: '害羞',
        content: '(✿◡‿◡)'
      }, {title: '睡啦', content: '(∪｡∪)..zzZ'}, {title: '臭美', content: '(o≖◡≖)'}, {title: '流汗', content: '(ーー゛)'}]
    }];
    ...
    this.w = this.iframe.contentWindow;
    this.d = this.iframe.contentDocument;
    this.d.open();
  ...
  ice.editor.prototype = {
    id: function (a) {
      ...
      range.insertNode(this.w.document.createDocumentFragment().appendChild(html));
      ...
      var box = el.getBoundingClientRect(), doc = el.ownerDocument, body = doc.body, html = doc.documentElement,
        ...
        top = box.top - clientTop, left = box.left - clientLeft;
  ...
  ice.editor.prototype.menuHTML = function () {
    ...
    this.createMenu({
      title: '富文本粘贴', name: 'paste', icon: 'word', success: function (e, z) {
    ...
    html += '</div></div>';
    this.createMenu({
      title: '插入表情', name: 'face', icon: 'face', dropdown: html, success: function (e, z) {
 ...
 `
    ...
    this.createMenu({
      ...
      success: function (e, z) {
        ...
        z.id(z.filesUploadId).onchange = function () {
          ...
          z.ajax({
            url: z.uploadUrl, data: formData, success: function (res) {
              if (res) {
    ...
    this.createMenu({
      ...
      success: function (e, z) {
        ...
        btn.onclick = function () {
          ...
          var domain = /^http(s)?:\/\/(.*?)\//.exec(url.value)
          if (domain.length == 3) {
            if (domain[2] == 'www.bilibili.com' || domain[2] == 'bilibili.com') {
              id = /video\/(.*?)\?/g.exec(url.value)
              if (!id) {
                id = /video\/(.*)/g.exec(url.value)
              }
              v.src = 'https://player.bilibili.com/player.html?bvid=' + id[1];
            } else if (domain[2] == 'v.youku.com') {
            ...
            } else if (domain[2] == 'www.ixigua.com') {
  ...
  ice.editor.prototype.pasteHTML = function (html) {
    html = html.replace(/<html.*?>/g, "").replace(/<body.*?>/g, "").replace(/<!.*?>/g, '').replace(/<head.*?>.*?<\/head>/g, '').replace(/<\/body>/g, '').replace(/<\/html>/g, '').replace(/\s+(id|class|lang|align|data|data-\w*)\s*=\s*(['"]?).*?\2/gi, '');
  ...
  ice.editor.prototype.formatHTML = function (html) {
    html = html.replace(/[\t\s]+/g, ' ').replace(/<(span|font|p|b|i|u|s)[^>]*>(<(?!img)[^>]*>)*<\/\1>/gi, '').replace(/<(.+)\s+>/g, '<$1>').replace(/(<br\/*>){1,}/gi, '<br/>');
  ...
  ice.editor.prototype.paste = function () {
    ...
    var upload = function (imgBase64) {
      ...
      _z.ajax({
        url: _z.uploadUrl, data: formData, success: function (res) {
          if (res) {
            for (var f = 0; f < res.length; f++) {
              if (res[f].error) {
    ...
    this.d.body.addEventListener('paste', function (e) {
      ...
      str = _z.getHTML();
      str = str.replace(/<p><\/p>/gi, '').replace(/<\/p><br>/gi, '').replace(/<(span|font|p|b|i|u|s)[^>]*>(<(?!img)[^>]*>)*<\/\1>/gi, '');
      _z.d.body.innerHTML = str;
      if (_z.imgAutoUpload) {
        var str = _z.getHTML();
        str.replace(/<img .*?src="(.*?)".*?>/gi, function (all, b = '') {
          (function (a) {
            ...
            _z.ajax({
              url: _z.uploadUrl, data: {'iceEditor-img': a}, success: function (res) {
                if (res && !res.error) {
  ...
  (function svg() {
    var c = document.createElement('style'), d, s, b = document.body;
    c.type = 'text/css';
    if (c.styleSheet) {
...
// 返回
  var index = window.iceEditor || ice.editor;
  return index;

})));

layui.define(function (exports) {
  exports('iceEditor', window.iceEditor);
});
