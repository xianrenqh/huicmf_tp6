...
z.ajax({url:z.uploadUrl,data:formData,success:function(res){if(res){for(var f=0;f<res.length;f++){if(res[f].error){z.filesUpload.error(res[f],res);z.filesUpload.complete(res[f],res);alert(res[f].error);}else{var a=z.c("a");a.href=res[f].url;a.className='download';a.download=res[f].name;a.innerText=res[f].name;a.target='_blank';z.setHTML(a,true);z.filesUpload.success(res[f],res);z.filesUpload.complete(res[f],res);}}
close.style.display='none';}else{z.filesUpload.error(res);z.filesUpload.complete(res);}},error:function(xhr){z.filesUpload.error(xhr);z.filesUpload.complete(xhr);}})}}});this.createMenu({title:'添加图片',name:'insertImage',icon:'pic',id:this.imageId,popup:{width:320,height:250,title:'图片上传',content:'<div class="iceEditor-insertImage"><div class="iceEditor-group"><label>URL：</label><input type="text" class="iceEditor-insertImageUrl" placeholder="网络图片地址" value=""/></div><div class="iceEditor-group"><label>宽：</label><input type="text" class="iceEditor-inputWidth" placeholder="宽" value=""/><label>高：</label><input type="text" class="iceEditor-inputHeight" placeholder="高" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div><input type="file" class="iceEditor-uploadInput" id="'+this.imgUploadId+'" name="file[]" accept="image/*" multiple/><div><svg class="iceEditor-icon iceEditor-uploadIcon" aria-hidden="true"><use xlink:href="#icon-pic"></use></svg></div><label for="'+this.imgUploadId+'" class="iceEditor-uploadBtn">点击上传图片</label></div>'},success:function(e,z){z.insertImage=z.id(z.imageId);var close=z.insertImage.getElementsByClassName('iceEditor-popup')[0];var url=z.insertImage.getElementsByClassName('iceEditor-insertImageUrl')[0];var width=z.insertImage.getElementsByClassName('iceEditor-inputWidth')[0];var height=z.insertImage.getElementsByClassName('iceEditor-inputHeight')[0];var btn=z.insertImage.getElementsByClassName('iceEditor-btn')[0];btn.onclick=function(){var img=z.c('img');img.src=url.value;if(width.value.trim())img.width=width.value.trim();if(height.value.trim())img.height=height.value.trim();z.setHTML(img);close.style.display='none';z.pd();}
z.id(z.imgUploadId).onchange=function(){if(!z.uploadUrl)return alert('请配置uploadUrl项');var formData=new FormData();for(var i=0;i<this.files.length;i++){formData.append('file[]',this.files[i]);}
z.ajax({url:z.uploadUrl,data:formData,success:function(res){if(res){for(var f=0;f<res.length;f++){if(res[f].error){z.imgUpload.error(res[f],res);z.imgUpload.complete(res[f],res);alert(res[f].error);}else{var a=z.c('img');a.src=res[f].url;z.setHTML(a,true);z.imgUpload.success(res[f],res);z.imgUpload.complete(res[f],res);}}
close.style.display='none';}else{z.imgUpload.error(res);z.imgUpload.complete(res);}}})}}});this.createMenu({title:'添加视频',name:'video',icon:'video',id:this.videoId,popup:{width:320,height:170,title:'添加视频',content:'<div class="iceEditor-video"><div><label>默认支持bilibili、优酷</label></div><div>URL：<input type="text" class="iceEditor-videoUrl" placeholder="网络图片地址" value=""/></div><div><label>宽：</label><input type="text" class="iceEditor-inputWidth" placeholder="px" value=""/><label>高：</label><input type="text" class="iceEditor-inputHeight" placeholder="px" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div></div>'},success:function(e,z){z.video=z.id(z.videoId);var type;var close=z.video.getElementsByClassName('iceEditor-popup')[0];var url=z.video.getElementsByClassName('iceEditor-videoUrl')[0];var width=z.video.getElementsByClassName('iceEditor-inputWidth')[0];var height=z.video.getElementsByClassName('iceEditor-inputHeight')[0];var btn=z.video.getElementsByClassName('iceEditor-btn')[0];btn.onclick=function(){if(!url.value.length)return alert('视频地址不能为空');var v=z.c('iframe');v.width=width.value.length?width.value:510;v.height=height.value.length?height.value:498;v.setAttribute('frameborder',0);v.setAttribute('allowfullscreen',true);var error='抱歉，无法处理该链接！';var domain=/^http(s)?:\/\/(.*?)\//.exec(url.value)
if(domain.length==3){if(domain[2]=='www.bilibili.com'||domain[2]=='bilibili.com'){id=/video\/(.*?)\?/g.exec(url.value)
if(!id){id=/video\/(.*)/g.exec(url.value)}
v.src='https://player.bilibili.com/player.html?bvid='+id[1];}else if(domain[2]=='v.youku.com'){var id=url.value.split('.html');if(id.length>1){id=id[0].split('id_');if(id.length>1&&id[1].length){v.src='https://player.youku.com/embed/'+id[1];}else{return alert('优酷：'+error);}}else{return alert('优酷：'+error);}}else if(domain[2]=='www.ixigua.com'){id=/[0-9]{6,}/g.exec(url.value)
v.src='https://www.ixigua.com/iframe/'+id[0];}else{v=z.c('video');v.src=url.value;v.width=width.value.length?width.value:510;v.height=height.value.length?height.value:498;v.controls='controls';}}else{return alert('URL地址不合法！');}
z.setHTML(v,true);close.style.display='none';z.pd();}}});this.createMenu({title:'最大化',name:'max',icon:'max',data:'max',css:'iceEditor-maxWindow'});this.createMenu({title:'最小化',name:'min',icon:'min',data:'min',css:'iceEditor-minWindow'});this.createMenu({name:'disabled',css:'iceEditor-disabledMask'});};ice.editor.prototype.menuFormat=function(){var _z=this;this.menuHTML();var ul=this.c('ul');ul.className='iceEditor-menu';this.tool.innerHTML='';this.tool.appendChild(ul);for(var i=0;i<this.menu.length;i++){if(this.menu[i]=='line'){var line=_z.c('li');line.className='iceEditor-line';ul.appendChild(line);continue;}
if(this.notMenu.indexOf(this.menu[i])==-1){ul.appendChild(this.menuList[this.menu[i]]);if(this.menuList[this.menu[i]].success){this.menuList[this.menu[i]].success(this.menuList[this.menu[i]],_z);}}}
if(this.maxWindow){ul.appendChild(this.menuList.max);ul.appendChild(this.menuList.min);}
ul.appendChild(this.menuList.disabled);this.editor.style.width=this.width;this.iframe.width=this.width;this.iframe.height=this.height;};ice.editor.prototype.menuAction=function(){var menu=this.tool.getElementsByClassName('iceEditor-exec');var _z=this;for(var i=0;i<menu.length;i++){menu[i].e=this;menu[i].attr=menu[i].getAttribute('data');if(menu[i].attr){menu[i].onclick=function(){switch(this.attr){case'strikeThrough':var parent=_z.range.anchorNode.parentNode;if(parent.style.textDecoration=='line-through'){var content=_z.range.anchorNode.parentNode.innerHTML;parent.parentNode.removeChild(parent);_z.setText(content,true);}else{var a=_z.c('span');a.style.textDecoration='line-through';_z.setHTML(a);}
...
break;case'max':var webHeight=window.innerHeight;if(typeof webHeight!='number'){if(document.compatMode=='CSS1Compat'){webHeight=document.documentElement.clientHeight;}else{webWidth=document.body.clientWidth;}}
_z.editor.style.position='fixed';_z.editor.style.zIndex=_z.getTime;_z.editor.style.width='100%';_z.editor.style.height='100%';_z.editor.style.top=0;_z.editor.style.left=0;_z.iframe.height=webHeight-35-20+'px';this.parentNode.style.display='none';_z.tool.getElementsByClassName('iceEditor-minWindow')[0].style.display='block';break;case'min':_z.editor.removeAttribute('style');_z.iframe.height=_z.height;this.parentNode.style.display='none';_z.tool.getElementsByClassName('iceEditor-maxWindow')[0].style.display='block';break;default:var b=this.attr.split('|');if(!_z.w.document._useStyleWithCSS){_z.w.document.execCommand('styleWithCSS',null,true);_z.w.document._useStyleWithCSS=true;}
if(b.length>1){_z.w.document.execCommand(b[0],false,b[1]);}else{_z.w.document.execCommand(b[0],false,null);}}
return false;}}}};ice.editor.prototype.pasteWord=function(html){function isWordContent(str){return/(class="?Mso|style="[^"]*\bmso\-|w:WordDocument|<(v|o):|lang=)/gi.test(str);
 }
 //转换cm/pt单位到px
 function unitToPx(v) {
